...
import { getConfigLayout } from '/src/helper/storage';
...
const ConfigTheme = (props) => {
    ...
    const setScale = (scale) => {
        setLayoutConfig((prevState) => ({ ...prevState, scale: scale }));
    };
    ...
    useEffect(() => {
        let layout = getConfigLayout();
        if (typeof layout === 'object' && Object.keys(layout).length > 0) {
            setScale(layout.scale);
            changeTheme(layout.theme, layout.colorScheme);
            changeInputStyle({ value: layout.inputStyle });
            changeRipple({ value: layout.ripple });
            changeMenuMode({ value: layout.menuMode });
        }
    }, []);

    return (
        <>
            <Sidebar {...props} position="right" className="layout-config-sidebar w-20rem">
                {!props.simple && (
                    <>
                        <h5>Scale</h5>
