...
const AppMenuitem = (props) => {
    ...
    const [active, setActive] = useState(activeMenu === key || activeMenu.startsWith(key + '-'));
    ...
    useEffect(() => {
        const newStr = router.pathname.replace(/\[.*$/, '').replace(/\/$/, '');
        // if (item.to && newStr === item.to) {
    ...
    useEffect(() => {
        setActive(activeMenu === key || activeMenu.startsWith(key + '-'));
    }, [activeMenu]);
        ...
        <li className={classNames({ 'layout-root-menuitem': props.root, 'active-menuitem': active })}>
            ...
            {item.to && !item.items && item.visible !== false ? (
                <Link href={item.to} replace={item.replaceUrl} target={item.target}>
                    ...
                    <a onClick={(e) => itemClick(e)} className={classNames(item.class, 'p-ripple', { 'active-route': active })} target={item.target} tabIndex="0">
                        <i className={classNames('layout-menuitem-icon', item.icon)}></i>
                        <span className="layout-menuitem-text">{item.label}</span>
                        {item.items && <i className="pi pi-fw pi-angle-down layout-submenu-toggler"></i>}
                        <Ripple />
