...
let url = '/api/v1/config/workflow';

export const getWorkflow = async (property) => {
...
export const getNextStatusWorkflow = async (property) => {
    var defaultParam = { ...property };
    var query_string = new URLSearchParams(defaultParam).toString();
    return new Promise((resolve) => {
        $axios
            .get(`${url}/next-status?${query_string}`)
            .then((result) => {
                let _data = result.data;
                return resolve(_data);
            })
...
export const confirmWorkflow = async (Params) => {
    return new Promise((resolve) => {
        $axios
            .post(`${url}-transition`, Params)
            .then((result) => {
                let _data = result.data;
                return resolve(_data);
            })
